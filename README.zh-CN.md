...
页面底部保留官方 [GitHub 项目](https://github.com/vaenshine/ic-atlas)链接，并显示页面版本。点击**更新**，可比较本地安装与官方项目的提交版本。每个浏览器、每个构建版本最多每 24 小时后台检查一次；点击**检查更新**可随时重新检查。检查会连接公开的 GitHub 项目，需要联网。

通过 Git 克隆并使用 `npm run dev` 或 `npm start` 启动后，可点击**更新本地版本**，阅读服务变更说明，再点击**安装并重启**。本地管理器会更新源码与依赖，为生产模式重新构建，然后重启页面服务。完成后重新加载实验台即可使用新版本，语言偏好继续保留。

更新从官方 `main` 分支获取代码，并使用快进合并。本地修改、未跟踪文件、其他分支、未完成的 Git 操作或分叉提交都会暂停安装并显示原因。请先保存自己的工作，再重新检查。派生仓库保留原有远程配置；独立提交需要手动与官方项目协调合并。

也可使用命令行更新。执行命令行安装前，请先停止正在运行的服务：
...
生产模式在命令行更新后执行 `npm run build` 和 `npm start`。依赖安装或构建失败时，更新面板会说明已完成的部分，并给出修复命令。更新管理器自身的新代码会在下次重新运行启动脚本、`npm run dev` 或 `npm start` 时生效。

ZIP 下载和普通静态托管通过**下载最新源码**或重新构建静态文件更新；在线 GitHub Pages 版本由仓库发布工作流更新。本地管理接口绑定回环地址，安装操作需要同源会话令牌。可用环境变量 `IC_ATLAS_PORT` 修改默认的 3000 端口。

早于 0.2.0 的安装可先执行一次 `git pull --ff-only` 和 `npm ci`，再通过 `npm run dev` 或 `npm start` 重启，获得更新功能；生产模式重启前需重新构建。
